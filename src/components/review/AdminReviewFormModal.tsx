...
const REVIEW_STATUSES: { value: ReviewStatus; label: string }[] = [
  ...
  { value: 'SELF_REVIEW_SUBMITTED', label: 'Auto-évaluation soumise' },
  { value: 'MANAGER_REVIEWED', label: 'Évalué par le manager' },
...
const AdminReviewFormModal: React.FC<Props> = ({ mode, review, employees, cycles, onClose, onSaved }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === 'create') {
      if (!employeeId) { setError('Veuillez sélectionner un employé.'); return; }
      if (!cycleId) { setError('Veuillez sélectionner un cycle d\'évaluation.'); return; }
    }
  ...
  const headerTitle = mode === 'create' ? 'Créer une évaluation (Admin)' : 'Modifier l\'évaluation (Admin)';
  const submitLabel = mode === 'create' ? 'Créer l\'évaluation' : 'Enregistrer les modifications';
    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[110] flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-xl shadow-2xl overflow-hidden animate-in zoom-in max-h-[90vh] flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-5 overflow-y-auto">
          ...
          {/* Mode édition : afficher le nom de l'employé en lecture seule */}
          {mode === 'edit' && review && (
          ...
          {/* Cycle d'évaluation (création uniquement) */}
          {mode === 'create' && (
            <div className="space-y-1">
              <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest px-1">Cycle d'évaluation</label>
              <select
                required
                className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl font-semibold text-sm outline-none focus:ring-4 focus:ring-primary-light transition-all"
                value={cycleId}
          ...
          {/* Manager N+1 */}
          <div className="space-y-1">
            <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest px-1">Manager N+1</label>
            <select
              className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl font-semibold text-sm outline-none focus:ring-4 focus:ring-primary-light transition-all"
              value={lineManagerId}
