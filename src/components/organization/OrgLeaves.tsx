  ...
  return (
    <div className="space-y-8 animate-in slide-in-from-bottom-8 duration-500">
       <div className="bg-white rounded-[3rem] border border-slate-100 shadow-sm overflow-hidden flex flex-col">
           <div className="p-8 bg-primary text-white flex items-center justify-between"><div className="flex items-center gap-4"><div className="p-3 bg-white/10 rounded-2xl"><ShieldCheck size={24}/></div><div><h3 className="text-xl font-semibold uppercase tracking-tight">Valeurs par défaut globales</h3></div></div></div>
       ...
       <div className="bg-white rounded-[3rem] border border-slate-100 shadow-sm overflow-hidden">
           <div className="p-8 bg-slate-800 text-white flex items-center justify-between">
              <h3 className="text-lg font-semibold uppercase tracking-wider">Dérogations par employé</h3>
              <button onClick={onAddOverride} className="px-4 py-2 bg-white/10 rounded-xl text-[10px] font-semibold uppercase tracking-widest flex items-center gap-2 hover:bg-white/20 transition-all"><Plus size={14}/> Ajouter une politique personnalisée</button>
           </div>
           <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
              {Object.entries(policy.overrides).map(([empId, quota]) => {
                 const empName = employees.find(e => e.id === empId)?.name || 'Utilisateur inconnu';
              ...
              {Object.keys(policy.overrides).length === 0 && <p className="col-span-full text-center text-slate-400 py-6 text-xs font-bold uppercase tracking-widest">Aucune dérogation individuelle définie.</p>}
           </div>
       </div>
    </div>
  );
